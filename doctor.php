...
$sql3 = "SELECT * FROM appointments WHERE doctorid = ? AND appointment_date = CURDATE()";   
...
//$row3['status'] = "Scheduled"; // Possible values: 'scheduled', 'completed', 'cancelled'\
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['approved'])) {
        ...
        $sql2 = "UPDATE appointments SET message  = 'Your appointment has been approved' WHERE appointment_id = ?";
        $stmt = $conn->prepare($sql);
        $stmt2 = $conn->prepare($sql2);
    ...
    if (isset($_POST['cancelled'])) {
        ...
        $sql2 = "UPDATE appointments SET message  = 'Your appointment has been cancelled' WHERE appointment_id = ?";
        $stmt2 = $conn->prepare($sql2);
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("i", $id);
        $stmt2->bind_param("i", $id);
    ...
    if (isset($_POST['scheduled'])) {
        $id =  $_POST['id'];
        $sql = "UPDATE appointments SET status = 'Scheduled' WHERE appointment_id = ?";
        $stmt = $conn->prepare($sql);
        $stmt->bind_param("i", $id);
        $stmt->execute();
    }
            ...
            <div class="col-lg-8">
                ...
                <div class="card dashboard-card">
                    <div class="card-header">
                        ...
                        <span class="ms-auto badge bg-primary" id="today-date">
                            <p id="today-date"></p>
                        </span>
                    </div>
                    <div class="card-body">
                        ...
                        ?>
                                <div class="appointment-item">
                                    ...
                                    <form method="post">
                                        ...
                                        <input type="hidden" name="id" value="<?php echo $row3['appointment_id']; ?>">
                                     <?php
                                        if( $row3['status'] == 'Scheduled' ) {
                                          echo "
                                            <button type='submit' name='approved' class='btn btn-success btn-sm'>Approve</button>
                                            <button type='submit' name='cancelled' class='btn btn-danger btn-sm'>Cancel</button>";
                                        } else if ($row3['status'] == 'Completed') {
